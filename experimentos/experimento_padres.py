# -*- coding: utf-8 -*-
"""
Created on Fri Oct  7 09:30:32 2022
...
def statisticals(tlabels, plabels, var= None,lstm = None ):
    ...
    results =	{"precision" : precision, "recall" : recall, "f_score" : f_score,
               "auc" : auc ,"aupr" : aupr, "fpr" : fpr, "fnr" : fnr, "padres" : var,"lstm" : lstm}
...
for v in padres: 
    df.append(pd.read_csv(f"DatosN_maxlag5maxp{v}var25.csv"))
    ground_truth.append(pd.read_csv(f"Ground_truthN_maxlag5maxp{v}var25.txt",sep = ",",header=None))
...
for k in range(len(padres)):
    ...
    df[k].columns = np.arange(0,var).tolist()
    df[k] = df[k].to_numpy()
...
for i in lstm:
    for k in range(len(padres)):
        ...
        #res.append(statisticals(ground_truth[k], val_matrix, var = padres[k], lstm = i))
        matrius.append(val_matrix)

matrius1 = matrius[0:6]
matrius2 = matrius[6:12]
matrius3 = matrius[12:18]
matrius4 = matrius[18:24]
matrius5 = matrius[24:30]
matrius6 = matrius[30:36]
res = []
...
for k in range(len(padres)):
    ...
    res.append(statisticals(ground_truth[k], matrius6[k], var = padres[k], lstm = lstm_))
...
df2.to_csv("resultados_exp_padres.csv", index= False)



#aviso de que ya ha termanido todo de ejecutrse
...
df_ = pd.read_csv('resultados_exp_padres.csv')
...
df_P.rename({'variables': 'Padres'}, axis=1, inplace=True)
n=len(df_T.columns)
fig,ax = plt.subplots(n-2,1, figsize=(10,n*3), sharex=True)

for i, col in enumerate(df_P.columns):
    if not (col ==  'lstm' or col == 'Padres'):
        plt.sca(ax[i])
        g = sns.scatterplot(data=df_P, x='lstm', y=col, hue='Padres', palette=['r', 'g', 'b','y','k','brown'])
        g.legend(loc='center left', bbox_to_anchor=(1.2, 0.5), ncol=1, title = 'Padres')
